...
- [ ] 2.2 Enforce superuser authorization and reject an empty or duplicate target list according to existing API conventions, returning a specific rejection reason for each validation failure
- [ ] 2.3 Validate every target, reject nonexistent IDs and the current user's ID before mutation, include the precise failure reason in the response, and preserve all-or-nothing transaction behavior
...
- [ ] 3.4 Add the count-based bulk confirmation dialog with loading spinner and disabled state during submission, automatic closing on response, cancel behavior, success and failure notifications that display the backend-provided rejection reason, query invalidation, and error handling
- [ ] 3.5 Add frontend tests for selection, current-user protection, current-page select-all, button state, confirmation, cancellation, loading spinner and disabled confirm state, automatic dialog close on success or failure, success notification, failure notification including the rejection reason, and successful deletion
